...
//TODO: The events or calls to the datagrid need to all be added to a queue. So if the grid is unable to do something then it will not loose it and can try it later. Even keep the last few that just went by so we know what just happened. This could ignore some updates if it knows there are more updates pending.
    ...
    function ListView(scope, element, attr, $compile) {
        ...
        // convenience
            options,
            states = ux.listView.states,
            events = ux.listView.events,
        // rendering
        ...
        // exports
            ...
            values = exports.values;// shortcut.
        ...
        function init() {
            ...
            exports.options = options = angular.extend({}, ux.listView.options, scope.$eval(attr.options) || {});
        }
        ...
        exports.start = function start() {
            exports.dispatch(ux.listView.events.INIT);
            flow.add(exports.templateModel.createTemplates);
            // if the templates have different heights. Then they are dynamic.
            flow.add(function updateDynamicRowHeights() {
                options.dynamicRowHeights = exports.templateModel.dynamicHeights();
            });
            flow.add(addListeners);
            flow.add(exports.setupScrolling);
        };
        ...
        function getRowElm(index) {
            return exports.chunkModel.getRow(index);
        }
...
flow.log("OVERWRITE DOM!!!");
            var len = list.length;
            flow.add(exports.chunkModel.chunkDom, [list, options.chunkSize, '<div class="ux-list-view-chunk">', '</div>', content], 0);
            exports.rowsLength = len;
            rowHeights = {};
        ...
        function compileRows(startIndex, limit) {
            ...
            limit = limit || exports.rowsLength;
            scope.$emit(events.BUILDING_PROGRESS, startIndex, exports.rowsLength);
        ...
        function compileRow(index) {
            ...
            if (!s) {
                ...
                tpl = exports.templateModel.getTemplate(exports.data[index]);
                if (prev) {
                    prev.$$nextSibling = s;
                    s.$$prevSibling = prev;
                }
                s.$status = 'compiled';
                s[tpl.item] = exports.data[index]; // set the data to the scope.
        ...
        function buildRows(list) {
            ...
            if (options.compileAllRowsOnInit) {
                flow.insert(compileRows, [0, exports.rowsLength]);
            }
        ...
        function getOffsetIndex(offset) {
            ...
            var est = Math.floor(offset/exports.templateModel.averageTemplateHeight()),
                i = 0;
            if (rowOffsets[est] && rowOffsets[est] <= offset) {
                i = est;
            }
            while (i < exports.rowsLength) {
        ...
        function updateAllHeights() {
            ...
            while (i < exports.rowsLength) {
                rowOffsets[i] = height;
                height += exports.templateModel.getTemplateHeight(exports.data[i]);
                i += 1;
            }
            options.rowHeight = exports.templateModel.getTemplateHeight('default');
        }
        ...
        function updateRowWatchers() {
            ...
            exports.dispatch(events.BEFORE_UPDATE_WATCHERS, loop);
            ...
            while (loop.i < exports.rowsLength) {
            ...
            updateLinks();
            exports.dispatch(events.AFTER_UPDATE_WATCHERS, loop);
            flow.add(safeDigest, [scope]);
        }
        ...
        function render() {
            if (state === states.BUILDING) {
                ...
                //flow.add(removeExtraRows, [exports.data]);// if our data updates we need to remove extra rows.
                viewHeight = element[0].offsetHeight;
                flow.add(buildRows, [exports.data], 0);
        ...
        function onDataChanged() {
            ...
            exports.data = exports.setData(scope.$eval(attr.uxListView || attr.list), scope.$eval(attr.grouped));
            flow.add(reset);
        }
        ...
        function reset() {
            ...
            exports.chunkModel.reset();
            state = states.BUILDING;
            flow.add(render);
        }

        exports.forceRenderScope = function forceRenderScope(index) {
            var s = scopes[index];
            if (!s && index > 0 && index < exports.rowsLength) {
                s = compileRow(index);
            }
            if (s) {
                activateScope(s);
        ...
        exports.dispatch = function dispatch() {
        ...
        function destroy() {
            ...
            flow.destroy();
            element[0].removeEventListener('scroll', onUpdateScroll);
