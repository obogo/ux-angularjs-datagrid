...
angular.module("ux").directive("uxDoubleScroll", function() {
    return {
        link: function(scope, element, attr) {
            var el = element[0], lastValue = 0, selector = scope.$eval(attr.uxDoubleScroll), target = document.querySelector(selector), isIOS = navigator.userAgent.match(/(iPad|iPhone|iPod)/g), vScroll, contentHeight = 0, elHeight = 0, scrollModel = scope.datagrid && scope.datagrid.scrollModel || {};
            function onScroll(event) {
                if (target) {
                    if (el.scrollTop + el.offsetHeight <= el.scrollHeight) {
                        scope.outerScrollTop = "hidden " + el.scrollTop + "/" + (el.scrollTop + el.offsetHeight) + "/" + el.scrollHeight;
                        target.style.overflow = "hidden";
                    } else {
                        scope.outerScrollTop = "scroll " + el.scrollTop + "/" + (el.scrollTop + el.offsetHeight) + "/" + el.scrollHeight;
            ...
            if (isIOS) {
                ...
                onScroll(null);
                scope.$on(exports.datagrid.events.READY, onSizeChange);
            }
