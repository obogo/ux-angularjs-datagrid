...
angular.module('ux').factory('sortModel', ['sortStatesModel', function (sortStatesModel) {
    ...
    return ['inst', function sortModel(inst) {
        ...
        result.applySorts = function applySorts(ary, sortOptions, clear) {
            var pathStateRef = sortStatesModel.getPathState(options[PATH]),
            ...
            if (options.sorts) {
                for (i in options.sorts) {
                    if (typeof options.sorts[i] === 'object') {
                        sortStatesModel.setState(i, options.sorts[i].value, pathState, options[PATH]);// value is the default sort state.
                        methods = options.sorts[i];// allow them to pass in their own sort methods.
                    } else {
                        if (!alreadyHasState) {
                            sortStatesModel.setState(i, options.sorts[i], pathState, options[PATH]); // set the default sort state.
                        }
                        methods = {
